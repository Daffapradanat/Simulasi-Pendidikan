...
// --- NAVBAR COMPONENT ---
export function Navbar({ user, onLogout, viewMode, inDetail, onNavigate }: { user: User | null, onLogout: () => void, viewMode: 'main' | 'profile' | 'result' | 'player', inDetail: boolean, onNavigate: (mode: 'main' | 'profile' | 'result' | 'player', resetModule_?: boolean) => void }) {
    ...
    <>
        ...
        {user && (
          <div className="navbar-menu">
             ...
             {(user.role === 'admin' || user.role === 'guru') && (
               <span className="navbar-link" onClick={() => navigate('/admin')} style={{ cursor: 'pointer' }}><i className="ti ti-layout-dashboard" aria-hidden="true"></i> Dashboard</span>
             )}
          </div>
        )}
        {user && (
      ...
      {user && mobileNavOpen && (
        <div className={`mobile-nav ${mobileNavOpen ? 'open' : ''}`}>
          ...
          {(user.role === 'admin' || user.role === 'guru') && (
             <span className="navbar-link" onClick={() => { setMobileNavOpen(false); navigate('/admin'); }}><i className="ti ti-layout-dashboard" aria-hidden="true"></i> Dashboard</span>
          )}
